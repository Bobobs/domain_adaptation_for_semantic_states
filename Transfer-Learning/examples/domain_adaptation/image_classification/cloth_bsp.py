...
def main(args: argparse.Namespace):
    ...
    #domain_PATH_train = "/home/gtzelepis/cropped_UR/domain_train/"
    ...
    #lr_scheduler = OneCycleLR(optimizer, 2e-3, epochs=args.epochs, steps_per_epoch=len(train_source_loader) + len(train_target_loader))
    # lr_scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=0.01, last_epoch=-1)
    ...
    if args.phase != 'train':
        checkpoint = torch.load(logger.get_checkpoint_path('best'+dev), map_location='cpu')
    ...
    if args.pretrain is None:
        ...
        args.pretrain = logger.get_checkpoint_path('pretrain'+ dev)
    ...
    for epoch in range(args.epochs):
        print("after pretraining lr:", lr_scheduler.get_last_lr()[0])
        # train for one epoch
        ...
        torch.save(classifier.state_dict(), logger.get_checkpoint_path('latest'+dev))
        if acc1 > best_acc1:
            shutil.copy(logger.get_checkpoint_path('latest'+dev), logger.get_checkpoint_path('best'+dev))
        best_acc1 = max(acc1, best_acc1)

    print("best_acc1 = {:3.1f}".format(best_acc1))

    # evaluate on test set
    classifier.load_state_dict(torch.load(logger.get_checkpoint_path('best'+dev)))
    acc1 = utils.validate(test_loader, classifier, args, device)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        total_domain_loss += transfer_loss
        total_bsp_loss += bsp_loss
        total_class_acc += cls_acc
    ...
    writer.add_scalar("Loss/Total Loss", total_loss, epoch)
    writer.add_scalar("Loss/Classification Loss", total_cls_loss, epoch)
    writer.add_scalar("Loss/BSP Loss", total_bsp_loss, epoch)
    writer.add_scalar("Loss/Domain Loss", total_domain_loss, epoch)
    writer.add_scalar("Accuracy/Classification Accuracy", total_class_acc, epoch)
    writer.add_scalar("Accuracy/Domain Accuracy", total_domain_acc, epoch)
