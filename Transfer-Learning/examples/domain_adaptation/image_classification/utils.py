...
def validate(val_loader, model, args, device) -> float:
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            # print(predicted)
            if i == 0:
                old_pred = predicted.cpu().numpy()
                old_pred2 = predicted2[:,1].cpu().numpy()
                old_pred3 = predicted3[:,2].cpu().numpy()
                old_pred4 = predicted9[:,3].cpu().numpy()
                old_pred5 = predicted9[:,4].cpu().numpy()
                old_pred6 = predicted9[:,5].cpu().numpy()
                old_pred7 = predicted9[:,6].cpu().numpy()
                old_pred8 = predicted9[:,7].cpu().numpy()
                old_pred9 = predicted9[:,8].cpu().numpy()
            else:
                old_pred = np.concatenate((old_pred, predicted.cpu().numpy()), 0)
                old_pred2 = np.concatenate((old_pred2, predicted2[:,1].cpu().numpy()), 0)
                old_pred3 = np.concatenate((old_pred3, predicted3[:,2].cpu().numpy()), 0)
                old_pred4 = np.concatenate((old_pred4, predicted9[:,3].cpu().numpy()), 0)
                old_pred5 = np.concatenate((old_pred5, predicted9[:,4].cpu().numpy()), 0)
                old_pred6 = np.concatenate((old_pred6, predicted9[:,5].cpu().numpy()), 0)
                old_pred7 = np.concatenate((old_pred7, predicted9[:,6].cpu().numpy()), 0)
                old_pred8 = np.concatenate((old_pred8, predicted9[:,7].cpu().numpy()), 0)
                old_pred9 = np.concatenate((old_pred9, predicted9[:,8].cpu().numpy()), 0)
            predictions.append(predicted.cpu().numpy())
            predictions2.append(predicted2[:,1].cpu().numpy())
            predictions3.append(predicted3[:,2].cpu().numpy())
            loss = F.cross_entropy(output, target)

            # measure accuracy and record loss
            acc1, = accuracy(output, target, topk=(1,))
            acc2, = accuracy(output, target, topk=(2,))
            acc3, = accuracy(output, target, topk=(3,))
            if confmat:
        ...
        # print(' * Acc@1 {top2.avg:.3f}'.format(top2=top2))
        if confmat:
            print(confmat.format(args.class_names))

    return top1.avg, old_pred, old_pred2, old_pred3, old_pred4, old_pred5, old_pred6, old_pred7, old_pred8, old_pred9
